                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
                    ...
                    <table style="font-size: 15px" id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody>
                      <?php
                      foreach ($getGelombang as $show) { ?>
                        <tr>
                          <th><?= $show->nama ?></th>
                          <td><?= 'Rp. '. number_format($show->harga,0,'','.') ?></td>
                          <td><?= date("d F Y",strtotime($show->mulai)) ?></td>
                          <td><?= date("d F Y",strtotime($show->akhir)) ?></td>
                          <td>
                            <div class="btn-group">
                              <a data-toggle="modal" href="#edit<?= $show->id_gelombang ?>"
                                  class="btn btn-primary btn-sm" style="width: 100px"><i class="fa fa-pencil"></i> Ubah</a>
                            </div>
                          </td>
                        </tr>
                        <!-- Small modal -->
                        <div id="edit<?= $show->id_gelombang ?>" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true" style="margin-top: 10vh">
                          <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                            <?= form_open_multipart('Administrasi/u_BiayaPPDB/'.$show->id_gelombang); ?>
                              ...
                              <div class="modal-body form-horizontal form-label-left">
                                ...
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="number" name="harga" class="form-control col-md-7 col-xs-12" required="required" value="<?= $show->harga ?>">
                                    </div>
                                </div>
                                <div class="item form-group">
                                    <label for="password" class="control-label col-md-3" style="text-align: right">Mulai (MM/DD/YYYY)</label>
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="date" name="mulai" class="form-control col-md-7 col-xs-12" required="required" value="<?= $show->mulai ?>">
                                    </div>
                                </div>
                                <div class="item form-group">
                                    <label for="password" class="control-label col-md-3" style="text-align: right">Sampai (MM/DD/YYYY)</label>
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="date" name="akhir" class="form-control col-md-7 col-xs-12" required="required" value="<?= $show->akhir ?>">
                                    </div>
                                </div>
                              </div>
                              <br>
